package io.piotrjastrzebski.box2dinterpolation;
 ...
 */
public class Box2dInterpolationScreen extends BaseScreen {
	...
	private TSType timeStepType = TSType.FIXED_INTERPOLATED;
	...
	public Box2dInterpolationScreen () {
		super();
		debugRenderer = new Box2DDebugRenderer();
		box2dWorld = new World(new Vector2(0, -10), true);
		largeBox = new Texture("box64.png");
		smallBox = new Texture("box32.png");
		smallCircle = new Texture("circle32.png");
		largeCircle = new Texture("circle64.png");
		createBounds();
		reset();
		createSettings();
		runSim(true);
	...
	private void createSettings () {
		...
		fpsSlider.addListener(new ChangeListener() {
			@Override public void changed (ChangeEvent event, Actor actor) {
				...
				fpsVal.setText(Integer.toString(targetFPS));
			}
		});
		...
		stepTimeSlider.addListener(new ChangeListener() {
			@Override public void changed (ChangeEvent event, Actor actor) {
				...
				stepTime = 1f / stepDiv;
				stepTimeVal.setText("1/" + (int)stepDiv);
			}
		});
		...
		c.add(pauseBtn);
		debugBtn = new VisTextButton("Show Debug", "toggle");
		...
		c.add(debugColor).colspan(3).row();
		...
		c.add(new VisLabel("Step type")).colspan(3).row();
		...
		VisTextButton stepInterpolated = new VisTextButton("F INTERPOLATED", "toggle");
	...
	private void debugDraw (boolean enabled) {
		debugDraw = enabled;
		if (debugDraw) {
			debugBtn.setChecked(true);
			debugBtn.setText("DebugDraw ON");
		} else {
			debugBtn.setChecked(false);
			debugBtn.setText("DebugDraw OFF");
		}
	}
	...
	private void createBounds () {
		float halfWidth = B2DInterp.VP_WIDTH / 2f - 0.5f;
		float halfHeight = B2DInterp.VP_HEIGHT / 2f - 0.5f;
...
//				fixedUpdate();
	...
	private class Box {
		...
		public Box (float x, float y, float rotation, Texture texture) {
			...
			width = srcWidth * B2DInterp.INV_SCALE;
			srcHeight = texture.getHeight();
			height = srcHeight * B2DInterp.INV_SCALE;
		}
